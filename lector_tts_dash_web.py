...
#  • pyttsx3 es OPCIONAL → si falta (Render) se salta la voz offline         #
#  • Todo en un solo archivo; drop-in para Heroku / Render / Fly.io          #
##############################################################################
...
# ── TTS libraries ───────────────────────────────────────────────────────────
try:
    import pyttsx3                       # disponible localmente
except ImportError:
    pyttsx3 = None                       # en la nube suele faltar

from gtts import gTTS                    # online fallback siempre disponible
...
VOICE_OPTIONS   = {"US English": "Zira"}     # key mostrado → substring en voz
DEFAULT_RATE    = 175
...
def smart_translate(text: str) -> str:
    ...
    translated = " ".join(
        GoogleTranslator(source=src_lang, target=tgt_lang).translate(s)
...
app.index_string = (
    ...
    "<nav class='navbar navbar-dark bg-danger sticky-top'><div class='container-fluid'>"
    "<span class='navbar-brand mb-0 h1'>🗣️ TTS Translator</span></div></nav>"
    "<div class='container-fluid pt-4'>{%app_entry%}</div>"
    "<footer class='text-center text-secondary py-4'><small>"
    "© 2025 STA methodologies · "
            ...
            dbc.Row([html.H5("Texto traducido", className="text-info mb-2"),
                     html.Pre(id="translation-box",
                              style={"whiteSpace":"pre-wrap",
                                     "fontSize":18,"minHeight":160})]),
            dbc.Row([html.H5("Lectura en curso",
...
def on_file_up(contents, filename):
    ...
    try: return extract_text(contents, filename)
    except Exception as e: return f"⚠️ Error leyendo archivo: {e}"
...
def on_dl_audio(text, toggle, _n):
    if not text or not text.strip(): return no_update
